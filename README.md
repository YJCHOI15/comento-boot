 ...
 **STM32F413ZHTX** MCU를 기반으로, RTOS 환경에서 전력 관리 반도체(**PMIC**)의 상태를 실시간으로 감시하고 차량 고장 진단 코드(**DTC**)를 관리하는 임베디드 펌웨어 프로젝트입니다.
 ...
 본 프로젝트는 **V-Model 프로세스**를 따라 체계적으로 진행되었으며, RTOS를 활용하여 여러 주변 장치를 안정적으로 동시에 제어하는 것을 목표로 합니다. 

 PMIC에서 감지된 **Fault** 정보와 ADC로 측정한 **전압 이상 상태**를 DTC로 변환하여 **EEPROM**에 기록하고, 외부 진단기의 **CAN 통신 요청**에 응답하여 진단 정보를 제공하는 차량용 제어기(ECU)의 핵심 기능을 구현했습니다.
...
향후 실제 하드웨어를 기반으로 각 모듈 및 전체 시스템의 기능과 안정성을 검증하는 단계를 진행할 예정입니다.
...
**IDE:** VS Code, STM32CubeIDE (빌드)

**통신 인터페이스:** UART, SPI, I2C, CAN

**타겟 부품:** MP5475GU(PMIC), 25LC256(EEPROM)
...
1.  PMIC의 각 Buck 채널이 개별 Fault 레지스터를 가지고 있다는 사실을 파악했기 때문에, 각 전원 라인별로 상세한 DTC를 매핑하고 하드웨어 제어를 담당하는 독립적인 드라이버를 만들 수 있었습니다. 
    즉, 좋은 아키텍처는 **하드웨어에 대한 깊은 이해**에서 비롯된다는 점을 느낄 수 있었습니다.

2.  이 프로젝트를 통해 RTOS의 가치는 **서로 다른 타이밍과 우선순위를 가진 여러 태스크들이 충돌 없이 안전하게 협력하도록 지휘하는 역할**에 있음을 이해했습니다.

    I2CTask는 1ms마다 주기적으로 움직이지만,
    SPITask와 CANTask는 외부의 이벤트가 발생할 때까지 Blocked 되어 CPU 자원을 아낍니다.

    이처럼 각기 다른 방식으로 동작하는 태스크들이 **뮤텍스, 세마포어, 메시지 큐**라는 도구를 통해 유기적으로 동작하도록 조율하는 것, 이것이 바로 RTOS를 사용하는 핵심적인 이유라는 인사이트를 얻었습니다.
